...
#不知道为啥这样写，有点冗余，调用只有一次，可以写的简单一点
#flatten()没有区别
#AdaptiveAvgPool2d在cpu上不支持没法测试
...
def rel_logits_1d(q, rel_k, permute_mask):
    ...
    rel_k = ops.transpose(rel_k, (1, 0)) #[dim, 2*window-1]

    #[batch,H,W,dim]*[dim, 2*window-1]=[batch,H,W,2*window-1]
    x = msnp.tensordot(q, rel_k, axes=1)

    x = ops.reshape(x, (-1, W, rel_size)) #[batch*H,W,2*window-1]
    ...
    x_pad = ops.reshape(x_pad, (-1, W+1, rel_size))    #[25088,9,27]
...
class HaloAttention(nn.Cell):
    ...
    def __init__(self,
                 ...
                 block_size=8,
                 halo_size=3,
                 qk_ratio=1.0, #ratio of q and k dimensions to output dimension when dim_head not set. 是attn_ratio?
                 qkv_bias=False,
    ...
    def construct(self, x):
        ...
        kv = kv_unfold(kv) # B, C_kh_kw, _, _
                ...
                ###########################
        #         kvB, kvdim, kvH, kvW = kv.shape

        #         sb = []
        #         for each_dim in range(kvdim):
        #             kvtmp = kv[:,each_dim]
        #             b = ops.reshape(kvtmp, (kvB, 1, kvH, kvW))
        #             output = kv_unfold(b)

        #             win_size = self.win_size * self.win_size

        #             output = ops.reshape(output, (kvB, win_size, -1))
      ...
      #  k, v = split(kv, [self.dim_head_qk, self.dim_head_v], axis=-1) #相当于按列划分
